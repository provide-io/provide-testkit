# env.ps1 - provide-testkit Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the provide-testkit root directory"
    exit 1
}
...
$Profile = if ($env:PROVIDE_TESTKIT_PROFILE) { $env:PROVIDE_TESTKIT_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/provide-testkit_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "provide-testkit_setup"
...
Write-Host "Installing provide-testkit in editable mode..." -NoNewline
try {
    & uv pip install --no-deps -e . 2>&1 | Out-File -FilePath (Join-Path $LogDir "install.log")
    Write-Success " provide-testkit installed"
}
catch {
...
Write-Host "`n$("provide-testkit development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  provide-testkit --help  # provide-testkit CLI"

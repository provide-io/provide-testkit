#!/bin/bash
#
# env.sh - provide-testkit Development Environment Setup
#
# This script sets up a clean, isolated development environment for provide-testkit
...
if [ ! -f "pyproject.toml" ]; then
    ...
    echo "Please run this script from the provide-testkit root directory"
...
PROFILE="${PROVIDE_TESTKIT_PROFILE:-default}"
if [ "$PROFILE" = "default" ]; then
    VENV_DIR="workenv/provide-testkit_${TFOS}_${TFARCH}"
else
...
mkdir -p /tmp/provide-testkit_setup

echo -n "Syncing dependencies..."
uv sync --all-groups > /tmp/provide-testkit_setup/sync.log 2>&1 &
SYNC_PID=$!
spinner $SYNC_PID
wait $SYNC_PID
SYNC_EXIT_CODE=$?
...
if [ $SYNC_EXIT_CODE -eq 0 ]; then
    ...
    echo "Check /tmp/provide-testkit_setup/sync.log for details"
    
    # Try to install just the project without dependencies first
    echo -n "Installing provide-testkit without dependencies..."
    uv pip install --no-deps -e . > /tmp/provide-testkit_setup/install_nodeps.log 2>&1 &
    INSTALL_PID=$!
    spinner $INSTALL_PID
    wait $INSTALL_PID
    if [ $? -eq 0 ]; then
        print_success "provide-testkit installed (no deps)"
    else
        print_error "Failed to install provide-testkit"
        return 1 2>/dev/null || exit 1
    fi
fi

echo -n "Installing provide-testkit in editable mode..."
uv pip install --no-deps -e . > /tmp/provide-testkit_setup/install.log 2>&1 &
spinner $!
print_success "provide-testkit installed"
# --- Sibling Packages ---
...
echo -e "\n${COLOR_GREEN}provide-testkit development environment activated${COLOR_NC}"
echo "Virtual environment: ${VENV_DIR}"
echo "Profile: ${PROFILE}"
echo -e "\nUseful commands:"
echo "  provide-testkit --help  # provide-testkit CLI"
...
find /tmp/provide-testkit_setup -name "*.log" -mtime +1 -delete 2>/dev/null
